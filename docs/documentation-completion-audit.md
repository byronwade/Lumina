...
| Docs are ready to guide prototype build work. | `docs/product-build-readiness.md`, `docs/phase-1-build-plan.md`, `docs/task-backlog.md`, `docs/package-map.md`, `docs/prototype-acceptance.md`, `docs/roadmap.md`, and feature contracts define the next path for broader component-level browser HMR and production hardening after the dev and production hydration proof, plus package boundaries, gates, acceptance criteria, risks, and evidence requirements without claiming broader behavior exists. | Covered for documentation readiness |
...
The root `bun run check` gate now runs documentation checks, structure checks, performance-documentation checks, TypeScript type checking, unit/integration tests, and the browser hydration smoke check. It verifies the current documentation, route discovery, generated manifest/map artifacts, route/inspect CLI paths, dev and production hydration bundle output, browser-verified interactive dev and production hydration, virtual route module behavior, route-file update reports, direct local imported component affected-route reports, static build/start output, and minimal dev-server guardrails. It does not prove SSR/API production behavior, broader component-level browser HMR, MCP tools, safe edits, or real benchmark results.
