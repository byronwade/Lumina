...
The root `bun run check` gate now runs documentation checks, structure checks, performance-documentation checks, TypeScript type checking, and tests. It verifies the current documentation, route discovery, generated manifest/map artifacts, route/inspect CLI paths, and minimal dev-server guardrails. It does not prove production build/start behavior, runtime adapters, route HMR, virtual route modules, client hydration, MCP tools, safe edits, or real benchmark results.
