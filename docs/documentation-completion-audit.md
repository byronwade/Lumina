...
| Features are consistent across docs and do not contradict current implementation status. | Root and public docs describe Phase 1 scaffold as Bun workspace, package placeholders, shared core types, initial compiler route discovery, CI, and enforcement scripts. They keep generated route artifacts, rendering, CLI behavior, runtime adapter behavior, Lumina Map generation, MCP tools, and safe edits marked planned. `bun run docs:check` rejects known stale pre-scaffold and unimplemented-behavior wording. | Covered for current docs system |
